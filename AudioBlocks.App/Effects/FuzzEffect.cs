...
namespace AudioBlocks.App.Effects
{
    ...
    public class FuzzEffect : IAudioEffect
    {
        ...
        private float dcBlock;
        private float dcPrev;
        ...
        public void Process(float[] buffer, int count)
        {
            ...
            // Low tone = ~300Hz cutoff (dark woolly), High tone = ~6kHz (bright)
            float toneFreq = 300f + Tone * 5700f;
            float lpCoeff = 1f - MathF.Exp(-2f * MathF.PI * toneFreq / 48000f);

            // High-pass to remove bass mud post-fuzz (~80Hz)
            float hpCoeff = 1f - MathF.Exp(-2f * MathF.PI * 80f / 48000f);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                // Positive half: harder clip (tanh squashed further)
                // Negative half: softer clip (allows more dynamics)
                float wet;
                if (x >= 0)
